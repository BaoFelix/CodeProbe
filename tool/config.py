...
# Design-review pass-1 fans its independent calls out concurrently
...
# hammered. Total wait is bounded by ceil(N/workers) × LLM_TIMEOUT; to cap
# it hard, lower LLM_TIMEOUT and/or raise LLM_MAX_WORKERS so all calls fire
# at once and resolve within one timeout window. (A soft "abandon
# stragglers past a deadline" mode was removed: a stock ThreadPoolExecutor's
# worker threads are non-daemon, so the process joins them at exit anyway —
# the deadline couldn't actually free the user. Per-call timeout + workers
# is the honest lever.)
try:
